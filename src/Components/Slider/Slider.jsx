...
const Slider = () => {
    ...
    const carouselImages = ['https://i.ibb.co/8g23ZCg/akhil-yerabati-4-Ym-SIWff6aw-unsplash.jpg', 'https://i.ibb.co/bJLqzwg/dhruv-vishwakarma-Eda49v-VHnp0-unsplash.jpg', 'https://i.ibb.co/80FWbv0/adrien-K12-Srka-Zu-Cg-unsplash.jpg', 'https://i.ibb.co/5jf5TwF/techietech-tech-J3-a1qdhvwg-unsplash.jpg', 'https://i.ibb.co/4m2pMwW/d-c-tr-nh-1t1-CBECuqvs-unsplash.jpg'];
    ...
    return (
        <div className="container rounded-lg shadow-md lg:mx-auto px-2 mt-10">
            <div className="h-60 w-full rounded-lg md:h-[470px] lg:h-[620px] relative overflow-hidden">
                {/* arrow left */}
                ...
                <div className="flex justify-center items-center rounded-full z-50 absolute bottom-4 w-full gap-1">
                    {carouselImages.map((_, inx) => (
                        <button key={_} onClick={() => setCurrentSlider(inx)} className={`rounded-full duration-2000 bg-blue-500 ${currentSlider === inx ? "w-8" : "wz-2"} h-2`}></button>
                    ))}
                </div>
                {/* Carousel container */}
                <div className="ease-linear duration-500 flex transform-gpu" style={{ transform: `translateX(-${currentSlider * 100}%)` }}>
                    ...
                    {carouselImages.map((slide, inx) => (
                        <img key={slide} src={slide} className="min-w-full h-60 bg-black/20 sm:h-96 md:h-[580px] object-cover" alt={`Slider - ${inx + 1}`} />
                    ))}
                </div>
            </div>
        </div>
